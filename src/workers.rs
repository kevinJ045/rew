...
pub fn op_thread_spawn(
  ...
  std::thread::spawn(move || {
      ...
      let runtime_result = rt.block_on(async {
          ...
          // Load worker source
          if let Err(e) = runtime.runtime.execute_script("<worker>", format!(r#"
...
pub fn op_thread_message(
  ...
  _: Rc<RefCell<OpState>>
    ...
    if let Some(handle) = registry.get(&thread_id) {
      handle.to_worker.send(serde_json::from_str(&msg).unwrap())
        .map_err(|e| CoreError::Io(std::io::Error::new(
...
pub fn op_thread_post_message(
  ...
  sender.send(msg).map_err(|e| CoreError::Io(std::io::Error::new(
