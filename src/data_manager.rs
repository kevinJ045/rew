...
impl DataManager {
    ...
    pub fn write(&self, key: &str, content: &str) -> Result<()> {
        ...
        fs::write(&path, content)
            .with_context(|| format!("Failed to write data file: {:?}", path))
    }
    ...
    pub fn delete(&self, key: &str) -> Result<()> {
        ...
        if path.exists() {
            fs::remove_file(&path)
                .with_context(|| format!("Failed to delete data file: {:?}", path))?;
        }
        Ok(())
    }
    ...
    fn list_recursive(&self, dir: &Path, result: &mut Vec<String>, prefix: &str) -> Result<()> {
        ...
        for entry in fs::read_dir(dir)
            ...
            if path.is_file() {
                let rel_path = path.strip_prefix(&self.data_dir)
                    .unwrap_or(&path)
                    .to_string_lossy()
                    .to_string();
    ...
    pub fn read_binary(&self, key: &str) -> Result<Vec<u8>> {
        let path = self.get_path(key);
        fs::read(&path)
            .with_context(|| format!("Failed to read binary data file: {:?}", path))
    }
    ...
    pub fn write_binary(&self, key: &str, data: &[u8]) -> Result<()> {
        ...
        fs::write(&path, data)
            .with_context(|| format!("Failed to write binary data file: {:?}", path))
    }
    ...
    fn detect_format(&self, path: &Path) -> Result<DataFormat> {
        ...
        if sample.iter().any(|&b| b < 9 || (b > 13 && b < 32 && b != 27)) {
