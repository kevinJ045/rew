  ...
  class RewExecutionContext {
    constructor(module){
      ...
      this.rew.prototype.mod.prototype.package = (packageName) => {
        ...
        if(path && path.length){
          let main = path.shift();
          let parent = SUB_PACKAGES.find(p => p.name == main) || this.rew.prototype.mod.prototype.package(main);
          path.forEach(p => {
            parent = parent.prototype[p] || this.rew.prototype.mod.prototype.package(p);
          });
      ...
      SUB_PACKAGES.filter((p) => p.packageName == module.app.config.manifest.package)
      .forEach((item) => {
      ...
      this.imp = async (filename) => {
        let [filepath, to_exec] = await ops.op_dyn_imp(filename.startsWith('/') ? '/' : module.filename, filename);
        _evalVM(to_exec);
      ...
      this.JSX = new Usage("jsx", (fn) => {
        this.__jsx__prefix = fn;
      })
  ...
  const _createRew = (...args) =>  _createClass({
    ...
    mod: _createClass({
      ...
      find(from, name, options = {}){
        ...
        const path = name.startsWith('#') || name.startsWith('/') ? name : 
                     name.startsWith('app://') ? name :
                     globalThis.rew.prototype._path.prototype.resolveFrom(fromPath, name);
