    ...
    constructor(filename) {
      this.filename = (() => {
        ...
        if(globalThis['__app__'+app_name]){
          return (globalThis['__app__'+app_name] + '/' + file_path).replaceAll('//', '/');
    ...
    constructor(module) {
      ...
      this.imp = async (filename) => {
        let [filepath, to_exec] = await ops.op_dyn_imp(
          filename.startsWith("/") ? "/" : module.filename,
          filename,
        );
        _evalVM(to_exec);
      ...
      this.genUid = function(length = 6){
        return ops.op_gen_uid(length);
      }
