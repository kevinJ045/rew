(function () {
    ...
    constructor(module){
      ...
      this.rew.prototype.ns = () => {
        return Namespace.group(
          ...
          this.rew.prototype.data  // Add data to the namespace
        );
      };

      // Set the module for data operations
      _log_out(module.app)
      this.rew.prototype.data.prototype.current_app = module.app || { config: {}, path: "" };
  ...
  const _createRew = () =>  _createClass({
    ...
    data: _createClass({
      // Basic operations
      read(key) {
        return ops.op_data_read(this.current_app.config.manifest.package, key);
      },
      ...
      async write(key, content) {
        ...
        return await ops.op_data_write(this.current_app.config.manifest.package, key, content);
      },
      
      async delete(key) {
        return await ops.op_data_delete(this.current_app.config.manifest.package, key);
      },
      
      exists(key) {
        return ops.op_data_exists(this.current_app.config.manifest.package, key);
      },
      
      list(prefix = '') {
        const result = ops.op_data_list(this.current_app.config.manifest.package, prefix);
      ...
      // YAML operations
      readYAML(key) {
        return ops.op_data_read_yaml(this.current_app.config.manifest.package, key);
      },
      
      async writeYAML(key, data) {
        return await ops.op_data_write_yaml(this.current_app.config.manifest.package, key, data);
      },
      
      // Binary operations
      readBinary(key) {
        const data = ops.op_data_read_binary(this.current_app.config.manifest.package, key);
        return new Uint8Array(data);
      },
      
      async writeBinary(key, data) {
        // Convert Uint8Array to regular array for serialization if needed
        const arrayData = data instanceof Uint8Array ? Array.from(data) : data;
        return await ops.op_data_write_binary(this.current_app.config.manifest.package, key, arrayData);
      },
      
      // Auto-detect format and read accordingly
      readAuto(key) {
        const [exists, format] = ops.op_data_get_info(this.current_app.config.manifest.package, key);
      ...
      // Write data with the appropriate format based on content and extension
      async writeAuto(key, data) {
        ...
        if (data instanceof Uint8Array || Array.isArray(data) && data.every(item => typeof item === 'number' && item >= 0 && item <= 255)) {
          ...
          // It's an object, use JSON or YAML based on extension
          if (ext === 'yaml' || ext === 'yml') {
