...
module.exports.prepareContext = function (
  ...
  context.require = (packageName, esm) => {
    try {
      const search = execOptions.nativeRequire || packageName.startsWith("node:")
        ? require(
            packageName.startsWith("node:")
              ? packageName.split("node:")[1]
              : packageName,
          )
        : customRequire(packageName, filepath, esm);
        if(!search) throw new Error("Module " + packageName + " not found");
        return search;
    } catch (e) {
      throw e;
    }
  };
  context.inc = (packageName, asserts) => {
    try {
      if (packageName.startsWith("node:") || packageName.startsWith("pkg:"))
        throw new Error("");
      return context.imp(packageName, asserts);
    } catch (e) {
      let pname = packageName.startsWith("pkg:") ? packageName.split("pkg:")[1] : packageName;
